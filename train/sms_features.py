...
def convert_value(value, max_val=None, min_val=None):
    ...
    try:
        ...
        warnings.warn('couldnt parse value to float (ValueError): ' + str(value), stacklevel=2)
        return 0.0
    except TypeError:
        warnings.warn('couldnt parse value to float (TypeError): ' + str(value), stacklevel=2)
        return 0.0
...
def font_size_bucket(font_size_raw):
    ...
    # bucket for > 40
    font_size = int(font_size_raw.rstrip('px'))
    if font_size < 8:
        return 'lt8'
    if 8 <= font_size < 25:
...
def get_features(row):
    ...
    # node type
    # allowed_types = ['a', 'b', 'blockquote', 'button', 'center', 'cite', 'code',
    #                 'dd', 'div', 'dt', 'em', 'fieldset', 'figcaption', 'footer', 'form',
    #                 'h1', 'h2', 'h3', 'h4', 'h5', 'h6', 'h7', 'h8', 'h9', 'header', 'i', 'iframe', 'label',
    #                 'legend', 'li', 'mark', 'nav', 'object', 'ol', 'option',
    #                 'p', 'pre', 'section', 'select', 'small', 'span',
    #                 'strong', 'sub', 'summary', 'sup', 'td', 'textarea',
    #                 'th', 'time', 'tt', 'u', 'ul', 'text', 'title']
...
def add_relative_features(row, agg, frm, ofrm, nn):
    ...
    # row['v_pos_clipped_rescaled'] = convert_value(rescale(row, agg, 'v_pos_clipped'))
    # row['h_pos_clipped_rescaled'] = convert_value(rescale(row, agg, 'h_pos_clipped'))
    ...
    # row['v_pos_clipped_standardised'] = convert_value(standardise(row, agg, 'v_pos_clipped'))
    # row['h_pos_clipped_standardised'] = convert_value(standardise(row, agg, 'h_pos_clipped'))
...
def enrich_features_groupwise(args):
    return enrich_features_groupwise_worker(*args)


def enrich_features_groupwise_worker(group_frame, raw_frame):
    frame = group_frame[1]
...
def get_features_groupwise(group_frame):
    frame = pd.DataFrame([get_features(row) for row in group_frame[1].to_dict(orient='records')],
                         index=group_frame[1].index.tolist())
    return frame  # .apply(add_relative_features, axis=1, agg=frame.describe(), frm=frame, ofrm=group_frame[1])
